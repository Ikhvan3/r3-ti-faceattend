...
export function ConfirmScheduleStatusDialog({
  schedule,
}: {
  ...
  const nextActive = !schedule.is_active;
  ...
  async function updateStatus() {
    ...
    try {
      const response = await fetch(
        `/api/admin/work-schedules/${schedule.id}/status`,
        {
          method: "PATCH",
      ...
      if (!response.ok || !payload.ok) {
        setError(
          safeScheduleApiMessage(
            response.status,
            payload.ok
              ? "Status jadwal kerja gagal diperbarui."
              : payload.message,
          ),
        );
    ...
    <>
      ...
      {isOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 px-4">
          <div
            aria-labelledby="schedule-status-dialog-title"
            aria-modal="true"
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-lg font-semibold text-slate-950" id="schedule-status-dialog-title">
                  {nextActive ? "Aktifkan jadwal" : "Nonaktifkan jadwal"}
                </h2>
                <p className="mt-1 text-sm text-slate-600">{schedule.name}</p>
              </div>
            ...
            <p className="mt-5 text-sm leading-6 text-slate-700">
              {nextActive
                ? "Jadwal kerja ini akan dapat digunakan lagi untuk penugasan pegawai."
                : "Jadwal kerja hanya dapat dinonaktifkan jika tidak sedang digunakan oleh assignment aktif atau masa depan."}
            </p>
