...
func (h Handler) VerifyForAttendance(w http.ResponseWriter, r *http.Request) {
	if !allowMethod(w, r, http.MethodPost) {
		return
	}
	claims, ok := auth.ClaimsFromContext(r.Context())
	...
	var req struct {
		Purpose          AttendanceVerificationPurpose `json:"purpose"`
		Embedding        []float64                     `json:"embedding"`
		EmbeddingModel   string                        `json:"embedding_model"`
		EmbeddingVersion string                        `json:"embedding_version"`
	}
	...
	result, err := h.service.VerifyForAttendance(r.Context(), claims, AttendanceVerificationInput{
		Purpose:          req.Purpose,
		Embedding:        req.Embedding,
		EmbeddingModel:   req.EmbeddingModel,
		EmbeddingVersion: req.EmbeddingVersion,
