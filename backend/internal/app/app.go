...
func newHTTPHandler(cfg config.Config, db health.DatabasePinger) http.Handler {
	...
	if ok && cfg.Auth.Validate() == nil {
		...
		if err == nil {
			...
			mux.Handle("/api/v1/admin/attendance/summary", adminOnly(http.HandlerFunc(adminAttendanceHandler.Summary)))
			mux.Handle("/api/v1/admin/attendance", adminOnly(http.HandlerFunc(adminAttendanceHandler.Collection)))
			mux.Handle("/api/v1/admin/attendance/", adminOnly(http.HandlerFunc(adminAttendanceHandler.Resource)))
